...
//  This class is originally the AsyncUdpSocket from CocoaAsyncSocket library
//  (distributed in the public domain).
//  Introduced in order to remove the third-party lib dependency in RevSDK.
//  Slightly modified, i.e. has some unnecessary code removed
//  and has some renames (otherwise SDK user may possibly have duplicate symbols).

#import <Foundation/Foundation.h>

@class RevAsyncSendPacket;
@class RevAsyncReceivePacket;

extern NSString *const RevAsyncUdpSocketException;
extern NSString *const RevAsyncUdpSocketErrorDomain;

typedef NS_ENUM(NSInteger, RevAsyncUdpSocketError) {
...
@interface RevAsyncUdpSocket : NSObject
{
	...
	RevAsyncSendPacket *theCurrentSend;
	NSTimer *theSendTimer;
	
	NSMutableArray *theReceiveQueue;
	RevAsyncReceivePacket *theCurrentReceive;
...
/**
 * Creates new instances of RevAsyncUdpSocket.
**/
...
@protocol RevAsyncUdpSocketDelegate
@optional

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)onUdpSocket:(RevAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)onUdpSocket:(RevAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error;
...
**/
- (void)onUdpSocket:(RevAsyncUdpSocket *)sock didNotReceiveDataWithTag:(long)tag dueToError:(NSError *)error;
